...
public class DataContextTests
{
    ...
    public void DataContext_RegisteredInServices()
    {
        var type = typeof(Program)
            .Assembly.GetTypes()
            .FirstOrDefault(x => x.IsSubclassOf(typeof(DbContext)));
        if (type == null)
        {
    ...
    public static List<dynamic> EnsureSet(string modelName, DbContext dataContext)
    {
        ...
        dynamic collection =
            generic?.Invoke(dataContext, null)
        ...
        {
            throw new Exception(
                $"it doesn't look like you registered '{modelName}' on your DbContext",
                e
            );
        }
        catch (Exception e)
        {
            throw new Exception(
                $"Attempting to call 'ToList' on your '{modelName}' set didn't work. Are you automatically migrating / seeding on startup?",
                e
            );
        }
    }
    ...
    private static Type GetEntityType(string entityTypeName)
    {
        try
        {
            var entityType = typeof(Program)
                .Assembly.GetTypes()
                .Single(x => x.Name == entityTypeName);
            return entityType;
        }
        catch (Exception e)
        {
    ...
    internal static DbContext? GetDataContext(IServiceScope scope)
    {
        try
        {
            var type = typeof(Program)
                .Assembly.GetTypes()
                .Single(x => x.IsSubclassOf(typeof(DbContext)));
